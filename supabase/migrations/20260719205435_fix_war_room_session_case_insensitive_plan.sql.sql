-- Make consume_war_room_session and get_workspace_war_room_usage
-- case-insensitive on p_plan / plan so 'Pro', 'PRO', 'pro' all resolve
-- to the Pro tier instead of falling through to the Free cap (which
-- caused the false "War Room session limit reached" 429 on Pro workspaces).
...
DECLARE
  ...
  v_plan         text;
BEGIN
  v_plan := lower(coalesce(p_plan, 'free'));
...
DECLARE
  ...
  v_overage      integer := 0;
BEGIN
  SELECT w.current_period_start, w.current_period_end, lower(coalesce(w.plan, 'free'))
