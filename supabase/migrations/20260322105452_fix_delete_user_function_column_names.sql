/*
  # Fix Delete User Function - Column Names
  
  1. Purpose
    - Fix the delete_user_account function to use correct column names
    - Updates references from incorrect column names to actual database columns
  
  2. Changes
    - posts: created_by → author_id
    - comments: created_by → author_id
    - challenges: created_by → creator_id
    - challenge_responses: created_by → user_id
    - assumption_forecasts: created_by → user_id
    - conversations: user1_id/user2_id → user_one_id/user_two_id
    - messages: receiver_id → (check if this exists)
