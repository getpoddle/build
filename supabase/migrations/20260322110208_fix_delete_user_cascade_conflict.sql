/*
  # Fix Delete User Function - Remove Cascade Conflicts
  
  1. Purpose
    - Fix ambiguous column reference by removing explicit deletes that CASCADE handles
    - Both moderation_actions and user_account_status have ON DELETE CASCADE
    - Explicitly deleting them before profiles causes conflicts
  
  2. Changes
    - Remove explicit DELETE from moderation_actions (CASCADE handles it)
    - Remove explicit DELETE from user_account_status (CASCADE handles it)
    - Remove explicit DELETE from content_reports (CASCADE handles it)
    - Let the foreign key CASCADE constraints do the cleanup
*/
...
BEGIN
  ...
  -- DO NOT explicitly delete moderation records - CASCADE will handle:
  -- - user_account_status (has ON DELETE CASCADE to profiles)
  -- - moderation_actions (has ON DELETE CASCADE to profiles)
  -- - content_reports (has ON DELETE CASCADE to profiles)
  ...
  -- Delete profile (CASCADE will handle user_account_status, moderation_actions, content_reports)
