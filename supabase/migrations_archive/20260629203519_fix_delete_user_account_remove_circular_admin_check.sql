...
-- Remove the admin check from the RPC — it causes a circular RLS dependency
-- (admins table policy calls is_admin() which queries admins).
-- Admin authorization is already enforced by the UI and the edge function.
...
BEGIN
  ...
  -- Profile (auth.users deletion is handled by the edge function)
