/*
  # Add Verification Eligibility Function

  1. New Functions
    - `check_verification_eligibility(user_id uuid)` - Checks if a user is eligible for verification
      - Returns eligibility status based on:
        - Email confirmed
        - Has first and last name
        - Has location
        - Account at least 7 days old
        - At least 3 posts or comments
      - Also checks if already verified or verification pending

  2. Purpose
    - Allows users to see their verification eligibility status
    - Provides detailed criteria checklist
    - Used by the verification modal UI
*/
...
BEGIN
  ...
  result := jsonb_build_object(
    'eligible', 
      ...
      (profile_data.city IS NOT NULL OR profile_data.country IS NOT NULL) AND
    ...
    'criteria', jsonb_build_object(
      ...
      'has_location', profile_data.city IS NOT NULL OR profile_data.country IS NOT NULL,
