...
BEGIN
  ...
  DELETE FROM workspace_invites        WHERE invited_by = target_user_id OR email IN (
    SELECT email FROM profiles WHERE id = target_user_id
  );
  DELETE FROM workspace_action_items   WHERE created_by = target_user_id OR assigned_to = target_user_id;
  ...
  DELETE FROM post_comment_ai_replies  WHERE post_id IN (SELECT id FROM posts WHERE user_id = target_user_id);
  DELETE FROM post_attachments         WHERE post_id IN (SELECT id FROM posts WHERE user_id = target_user_id);
  ...
  DELETE FROM moderation_actions       WHERE moderator_id = target_user_id OR target_user_id = target_user_id;
  ...
  DELETE FROM password_reset_attempts  WHERE user_id = target_user_id;
