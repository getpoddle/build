/*
  ...
  ## Overview
  Transforms Pods from communities into structured strategic reasoning environments.
  Each Pod becomes a strategic question with a 5-tab analysis framework.

  ## Changes

  ### Modified Tables
  - `pods`: Added time_horizon, domain, question_text, forecast_count, avg_probability, disagreement_level

  ### New Tables
  1. `pod_assumptions` - Base assumptions for a strategic question
     - content: the assumption text
     - challenge_count: how many users have challenged it

  2. `assumption_challenges` - User challenges to assumptions
     - user can challenge any assumption with a written response

  3. `pod_options` - Strategic options/paths for the question
     - title, description, implementation_notes, capital_notes

  4. `pod_risks` - Risks attached to the pod or specific options
     - category: execution | political | capital | other
     - severity: 1-5 scale

  5. `pod_forecasts` - User probability forecasts
     - probability: 0-100 integer
     - justification: required text
     - UNIQUE per user per pod (upsert)

  ## Security
  - RLS enabled on all new tables
  - Authenticated users can read all; only authors/forecasters can modify their own
*/
...
-- Function to update pod forecast stats and disagreement level
...
BEGIN
  ...
  -- Normalize stddev to 0-100 disagreement scale (max stddev is ~50 for uniform distribution)
...
CREATE OR REPLACE TRIGGER on_forecast_change
...
CREATE OR REPLACE TRIGGER on_challenge_change
