...
import { buildPaymentFailedEmail } from "../_shared/emailTemplates.ts";
...
Deno.serve(async (req: Request) => {
  ...
  try {
    const { userId, planLabel, amountFormatted, billingPortalUrl } = await req.json();
    ...
    const html = buildPaymentFailedEmail(firstName, plan, amountFormatted || "—", portalUrl);
    ...
    const res = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        ...
        to: email,
        subject: `Action required — payment failed for your Poddle ${plan} subscription`,
        html,
      }),
    });
    ...
    console.log(`Payment-failed email sent: ${result.id} → ${email}`);
    ...
    console.error("send-payment-failed error:", msg);
